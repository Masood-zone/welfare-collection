...
const getWelfarePayments = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { welfareId } = req.params;
    try {
        const payments = yield (0, payments_helper_1.getWelfarePaymentsHelper)(welfareId);
        res.status(200).json(payments);
    }
    catch (error) {
...
const updatePaymentAmount = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { paymentId } = req.params;
    const { amount } = req.body;
    // Check if payment exists
    const payment = yield (0, payments_helper_1.findPaymentByIdHelper)(paymentId);
    // If payment does not exist, return an error
    if (!payment) {
        return next(new app_error_1.AppError("Payment not found", 404));
    }
    try {
        const { updatedPayment, updatedPaymentTracker } = yield (0, payments_helper_1.updatePaymentRemainingAmountHelper)(paymentId, amount);
        res.status(200).json({
