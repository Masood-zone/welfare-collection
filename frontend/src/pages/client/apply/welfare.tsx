...
export default function ApplyWelfare() {
  ...
  const handleEnroll = async () => {
    try {
      await enroll({ userId, welfareProgramId: welfareProgram.id });
    } catch (error) {
      console.error(error);
    }
  };
    ...
    <div className="max-w-3xl mx-auto p-4">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          ...
          {/* <p>Created by: {welfareProgram?.createdByUser?.name}</p> */}
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <p className="mb-4">
            By enrolling in this program, you agree to pay the stated amount of{" "}
            <span className="text-lg font-bold text-primary">
              Ghc{welfareProgram?.amount}{" "}
              {welfareProgram?.paymentCycle?.toLowerCase()}
            </span>
            . Please read and agree to the terms before enrolling.
          </p>
        ...
        <CardFooter>
          <Button
            ...
            disabled={!agreeToTerms || enrolling}
            className="w-full"
          >
